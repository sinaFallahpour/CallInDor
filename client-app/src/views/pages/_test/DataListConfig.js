...
class DataListConfig extends Component {
  ...
  async populatingData() {
    ...
    const { data } = await agent.Test.list(params);
    if (data?.result) {
      await this.setState({
        data: data.result.data.tests,
        allData: data.result.data.tests,
  ...
  handleFilter = async (e) => {
    if (this.state.value == "" && e.target.value == "")
      return
    await this.setState({ loading: true, value: e.target.value, currentPage: 0 })
    this.populatingData();
  };

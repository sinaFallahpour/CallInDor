...
class ModalForm extends Form {
  ...
  async populatingResources() {
    ...
    const { data } = await agent.Resources.StaticWordList(this.state.languageHeader.enumVal);
    let staticWords = data.result.data;
    this.setState({ staticWords, loadingData: false });
  }
  ...
  handleOnChange = (event, index) => {
    var staticWords = this.state.staticWords;
    var foundIndex = staticWords.findIndex(x => x.name == event.target.name);
    staticWords[foundIndex] = { name: event.target.name, value: event.target.value };
    this.setState({ staticWords })
  }
  ...
  doSubmit = async (e) => {
    ...
    try {
      const { staticWords, languageHeader } = this.state;


      var rv = { languageHeader: languageHeader.enumVal };
      for (var i = 0; i < staticWords.length; ++i)
        rv[staticWords[i].name] = staticWords[i].value;


      // var dddddd = {}
      // staticWords.forEach(function (data) {
      //   Object.defineProperty(dddddd, data.name, {
      //     value: data.value
      //   })
      // });
      const { data } = await agent.Resources.editStaticWordList(rv, languageHeader.header);
  ...
  render() {
    const { staticWords, loadingData } = this.state;
    if (loadingData) {

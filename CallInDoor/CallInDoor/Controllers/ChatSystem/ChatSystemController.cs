...
namespace CallInDoor.Controllers.ChatSystem
{
    public class ChatSystemController : BaseControlle
    {
        ...
        public async Task<ActionResult> GetAllChatsList()
        {
            ...
            var myClientservices = _context.BaseRequestServiceTBL
                                .Where(c =>
                             ...
                             c.ServiceTypes.Contains("0")
                                ...
                                .Include(c => c.ChatRequestTBL)
                                //////.Include(c => c.ChatServiceMessagesTBL)
                                .Include(c => c.ChatForLimitedServiceMessagesTBL)
                                .AsQueryable().ToList();

            var myProviderservices = _context.BaseRequestServiceTBL
                                .Where(c =>
                                    c.ServiceRequestStatus == ServiceRequestStatus.Confirmed &&
            ...
            var myClientRequestJoinWithUser = await (from c in myClientservices
                                                     ...
                                                     select new AllSericesVM
                                                     {
                                                         ...
                                                         UnreadMessageCount =
                                                            c.ChatRequestTBL.IsLimitedChat ?
                                                         ...
                                                         Name = u.Name,
                                                         LastName = u.LastName,
                                                         IsPerodedOrsesionChat = c.ChatRequestTBL.IsLimitedChat,
                                                         Id = c.Id
        ...
        public async Task<ActionResult> SeenAllLimitedChats(int? requestId)
        {
            ...
            var chats = await _context.ChatForLimitedServiceMessagesTBL.Where(c => c.BaseServiceRequestId == requestId && c.IsSeen == false
            ...
            var requestfromDB = await _context.BaseRequestServiceTBL.AsNoTracking().Where(c => c.Id == requestId)
        ...
        /// <summary>
        /// درخواست به سرویسی که از جنس چت است    
        /// </summary>
        /// <param name="model"></param>
        /// <returns></returns>
        [HttpPost("RequestToChat")]
        //[Authorize]
        [ClaimsAuthorize(IsAdmin = false)]
        public async Task<ActionResult> RequestToChat([FromBody] RequestToChatssDTO model)
        {
            ...
            var hasReserveRequest = await _context.BaseRequestServiceTBL.AnyAsync(c =>
                                ...
                                ||
                                //سرویس هایی که تایید شده اند و یا پایان نشده زمانش و تعدادی پیام استفاده شده کمتر از اعداد کل پیام هایه خریداری شده است
                                (c.ServiceRequestStatus == ServiceRequestStatus.Confirmed /*|| c.ServiceRequestStatus == ServiceRequestStatus.Pending*/)
                                &&
                            (/*c.HasPlan_LimitedChatVoice == true*/ c.ChatRequestTBL.UsedMessageCount_LimitedChat < c.ChatRequestTBL.AllMessageCount_LimitedChat &&
                                                    c.ChatRequestTBL.ExpireTime_LimitedChatVoice < DateTime.Now /*c.IsExpired_LimitedChatVoice == false*/) &&
                                         /*c.ServiceRequestStatus == ServiceRequestStatus.Pending &&*/
                                         c.BaseServiceId == model.BaseServiceId &&
                                         c.ClienUserName == currentUsername &&
                                           //////c.ServiceType == ServiceType.ChatVoice &&
            ...
            var baseRequestServiceTBL = new BaseRequestServiceTBL()
            {
                ...
                LastTimeTheMessageWasSent = DateTime.Now,
            ...
            var chatRequest = new ChatRequestTBL()
            {
                IsLimitedChat = true,
                BaseRequestServiceTBL = baseRequestServiceTBL,

                AllMessageCount_LimitedChat = baseServiceFromDB.MyChatsService.MessageCount != null ? (int)baseServiceFromDB.MyChatsService.MessageCount : 0,
                UsedMessageCount_LimitedChat = 0,
                //ExpireTime_LimitedChatVoice = baseServiceFromDB.MyChatsService.Duration
                ExpireTime_LimitedChatVoice = DateTime.Now.AddDays(10),
            };


            baseRequestServiceTBL.ChatForLimitedServiceMessagesTBL = new List<ChatForLimitedServiceMessagesTBL>() {
            ...
            await _context.BaseRequestServiceTBL.AddAsync(baseRequestServiceTBL);
            await _context.SaveChangesAsync();
            return Ok(_commonService.OkResponse(null, false));
        ...
        /// <summary>
        /// ******************* changed to new Api
        ///قبول کردن سرویس که برای همه نوع سرویس های چت میشود
        /// </summary>
        /// <param name="requestId"></param>
        /// <returns></returns>
        [HttpGet("AcceptChatRequest")]
        [ClaimsAuthorize(IsAdmin = false)]
        public async Task<ActionResult> AcceptChatRequest(int requestId)
        {
            var currentUserName = _accountService.GetCurrentUserName();
            ...
            if (requestFromDB.ServiceRequestStatus != ServiceRequestStatus.Pending)
            {
                List<string> erros = new List<string> {
            ...
            requestFromDB.ChatRequestTBL.AcceptDate = DateTime.Now;
        ...
        /// ******************* changed to new Api
        [HttpGet("RejectChatRequest")]
        [ClaimsAuthorize(IsAdmin = false)]
        public async Task<ActionResult> RejectChatRequest(int requestId)
        {
            var currentUserName = _accountService.GetCurrentUserName();
            var requestFromDB = await _context.BaseRequestServiceTBL
                                           .Where(c => c.Id == requestId && c.ProvideUserName == currentUserName)
        ...
        public async Task<ActionResult> GetSeviceDatilsByRequestId(int requestId)
        {
            ...
            var requestFromDB = await _context.BaseRequestServiceTBL.Where(c => c.Id == requestId).FirstOrDefaultAsync();
        ...
        public async Task<ActionResult> GetClientInFoWithRequestId(int requestId)
        {
            ...
            var requestFromDB = await _context.BaseRequestServiceTBL.Where(c => c.Id == requestId &&
        ...
        public async Task<ActionResult> GetChatsDetails(int requestId)
        {
            ...
            var chats = await _context.ChatForLimitedServiceMessagesTBL /*ChatServiceMessagesTBL*/
                .Where(c => c.BaseServiceRequestId /*ServiceRequestId */== requestId
                    && (c.ClientUserName == currentUsername || c.ProviderUserName == currentUsername))
                ...
                .Select(c => new
                {
                    ...
                    IsMyMessage = IsMyMessage(c.BaseRequestServiceTBL /*ServiceRequestTBL*/, c.IsProviderSend, currentUsername, c.ClientUserName, c.ProviderUserName),
                    c.ChatMessageType,
                    c.IsSeen,
        ...
        [NonAction]
        private static bool IsMyMessage(BaseRequestServiceTBL baseRequestServiceTBL, bool isProviderSend, string currentUserName, string clientUserName, string ProviderUserName)
        {
            if (baseRequestServiceTBL?.ClienUserName == currentUserName)
            {
                if (isProviderSend)
                    return false;
                return true;
        ...
        public async Task<ActionResult> GetChatInfo(bool isLimited, int requestId)
        {
            ...
            var requestFromDB = await _context.BaseRequestServiceTBL.Where(c => c.Id == requestId)
                  ...
                  {
                      IsProvider = c.ProvideUserName == currentUsername ? true : false,
                      remainedMessageCount = c.ChatRequestTBL.AllMessageCount_LimitedChat - c.ChatRequestTBL.UsedMessageCount_LimitedChat,
                      c.ChatRequestTBL.UsedMessageCount_LimitedChat,
                      c.ChatRequestTBL.ExpireTime_LimitedChatVoice,
                      c.ChatRequestTBL.AllMessageCount_LimitedChat,
                      c.ProvideUserName,
                      c.ClienUserName,
                  })
                  .FirstOrDefaultAsync();
        ...
        public async Task<ActionResult> GetLimitedChatDetails(int requestId)
        {
            ...
            var chats = await _context.ChatForLimitedServiceMessagesTBL
                .Where(c => c.BaseServiceRequestId == requestId
                    && (c.ClientUserName == currentUsername || c.ProviderUserName == currentUsername))
                .OrderBy(c => c.CreateDate)
                .Select(c => new
                {
                    ...
                    c.Text,
                    SendetMesageType = c.SendetMesageType,
                    IsMyMessage = IsMyMessage(c.BaseRequestServiceTBL, c.IsProviderSend, currentUsername, c.ClientUserName, c.ProviderUserName),
                    c.ChatMessageType,
                    c.IsSeen,
                    c.FileOrVoiceAddress,
                })
        ...
        public async Task<ActionResult> HasLimitedPlanForServcie(int requestId)
        {
            ...
            {
                if (/*requestFromDB.HasPlan_LimitedChatVoice &&*/ requestFromDB.ChatRequestTBL.ExpireTime_LimitedChatVoice > DateTime.Now &&
                                   requestFromDB.ChatRequestTBL.UsedMessageCount_LimitedChat < requestFromDB.ChatRequestTBL.AllMessageCount_LimitedChat)
                ...
                //)
                {
                    var response = new
                    {
                        isExpired = requestFromDB.ChatRequestTBL.ExpireTime_LimitedChatVoice > DateTime.Now /* requestFromDB.IsExpired_LimitedChatVoice*/,
                        ////////hasPlan = requestFromDB.HasPlan_LimitedChatVoice,
                        isProvider = currentUsername == requestFromDB.ProvideUserName,
                        remaingChat = requestFromDB.ChatRequestTBL.AllMessageCount_LimitedChat - requestFromDB.ChatRequestTBL.UsedMessageCount_LimitedChat,
                        result = true,
                    };
                    allResponse = true;
                    return Ok(_commonService.OkResponse(response, false));
                ...
                var res = new
                {
                    isExpired = requestFromDB.ChatRequestTBL.ExpireTime_LimitedChatVoice > DateTime.Now,
                    //hasPlan = requestFromDB.ChatRequestTBL.HasPlan_LimitedChatVoice,
                    isProvider = currentUsername == requestFromDB.ProvideUserName,
                    remaingChat = requestFromDB.ChatRequestTBL.AllMessageCount_LimitedChat - requestFromDB.ChatRequestTBL.UsedMessageCount_LimitedChat,
                    result = false,
                };
        ...
        /////// <summary>
        /// in delete shodeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee fuck baba
        /// 
